class ArticleList {
  constructor() {
    ...
    this.articlesService = new DataService("api/articles.json");
    this.renderArticles();
  }
  // Render articles
  async renderArticles() {
    let articleListDomString = "";
    const articles = await this.articlesService.getData();
  ...
  async showArticle(event) {
    ...
    const article = await this.articlesService.getDataItemById(id);
